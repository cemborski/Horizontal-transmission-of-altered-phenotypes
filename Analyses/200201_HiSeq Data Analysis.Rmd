...
resultsNames(ddse1)
#Contrasts:
res0 <- results(ddse1, contrast=c("GenTreat","F0CD","F0NSD")) #numerator (control) , denominator(nsd)
res1 <- results(ddse1, contrast=c("GenTreat","F1CD","F1NSD"))
...
resultsNames(ddse2)
#Contrasts:
res2 <- results(ddse2, contrast=c("GenTreat","F2CD","F2NSD")) 
res3 <- results(ddse2, contrast=c("GenTreat","F3CD","F3NSD"))
...
bind_rows(as.data.frame(res0)[,c("genus.D5", "log2FoldChange", "pvalue")],
          as.data.frame(res1)[,c("genus.D5", "log2FoldChange", "pvalue")],
          as.data.frame(res2)[,c("genus.D5", "log2FoldChange", "pvalue")],
          as.data.frame(res3)[,c("genus.D5", "log2FoldChange", "pvalue")],
          .id = "generations") %>% mutate(generations = as.numeric(generations) - 1, sig = ifelse(padj<0.05, 1, .5)) %>%
  ggplot(aes(generations, log2FoldChange, color = genus.D5, alpha = sig, group = genus.D5)) + geom_point(aes(size=5)) + geom_line(alpha=0.5)
...
```{r}
#To be completed after lines 263-266 are resolved
```
